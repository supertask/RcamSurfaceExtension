...
{
    [ExecuteInEditMode]
    sealed class RcamSurface6 : MonoBehaviour
    {
        ...
        //
        [Space]
        //[SerializeField, Range(0.25f, 10.0f)] float _fresnelExponent = 0.25f;
        [SerializeField] float _fresnelExponent = 4.0f;
        [SerializeField] Color _fresnelColor = new Color(0, 0, 1, 1);
        [SerializeField] float _binaryThreshold = 0.0001f;

        public float fresnelExponent { set { _fresnelExponent = value; } }
        public Color fresnelColor { set { _fresnelColor = value; } }
        public float binaryThreshold { set { _binaryThreshold = value; } }
        ...
        void LateUpdate()
        {
            ...
            _props.SetFloat("_BinaryThreshold", _binaryThreshold);

            var tref = _sensorOrigin != null ? _sensorOrigin : transform;
            _props.SetMatrix("_LocalToWorld", tref.localToWorldMatrix);
            _props.SetMatrix("_WorldToCamera", Camera.main.worldToCameraMatrix);
