...
sampler2D _HoloLinesMap;
float4 _HoloLinesMap_ST;
...
)
{
	...
	float4 glitch_ver0 = glitch(uv0.xy)*(0.001 * _deform) * mask_map0;
	float4 glitch_ver1 = glitch(uv1.xy)*(0.001 * _deform) * mask_map1;
	float4 glitch_ver2 = glitch(uv2.xy)*(0.001 * _deform) * mask_map2;
	float4 glitch_ver3 = glitch(uv3.xy)*(0.001 * _deform) * mask_map3;
