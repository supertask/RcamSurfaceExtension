...
float4 _WireframeTiling;
float4 _WireframeOffset;
...
)
{
	...
	//Dissolve
	//float4 c = tex2Dlod(_DissolveTex, uv0);
	//half clipping = tex2Dlod(_DissolveTex, float4(uv0.x, uv0.y, 0, 0)).r - _DissolvePercentage;
	/*
	clip(clipping);
	clipping = tex2Dlod(_DissolveTex, uv1).rgb - _DissolvePercentage;
	clip(clipping);
	clipping = tex2Dlod(_DissolveTex, uv2).rgb - _DissolvePercentage;
	clip(clipping);
	clipping = tex2Dlod(_DissolveTex, uv3).rgb - _DissolvePercentage;
	clip(clipping);
	*/
