            ...
            )
{
	...
	half clipping = tex2Dlod(_DissolveTex, float4(uv * _DissolveTiling + _DissolveOffset, 0, 0)).r - _DissolvePercentage;
	float4 wireframeSampleTex = tex2Dlod(_WireframeTex, float4(uv * _WireframeTiling + _WireframeOffset, 0, 0));
	float4 wireframeColor = _WireframeColor;
	half wireframeAlpha = -1 * clipping + 0.9; //Clip gradually by alpha of wireframe

	if (0 < clipping && clipping < _LineWidth && _DissolvePercentage > 0) {
		builtinData.emissiveColor = _LineColor; 
	}
	else if (_LineWidth < clipping) {
		builtinData.emissiveColor = surfaceData.baseColor * 15; 
	}
	else if (clipping < 0) {
