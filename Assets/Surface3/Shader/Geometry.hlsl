...
)
{
	...
	//
	// Custom: Pru Pru vertex shader
	//
	half4 d0 = tex2Dlod(_DispTex, float4(p0.x, p0.y + _Time.y, 0, 0));
	half4 d1 = tex2Dlod(_DispTex, float4(p1.x, p1.y + _Time.y, 0, 0));
	half4 d2 = tex2Dlod(_DispTex, float4(p2.x, p2.y + _Time.y, 0, 0));
	half4 d3 = tex2Dlod(_DispTex, float4(p3.x, p3.y + _Time.y, 0, 0));
	p0 += _Displacement * n0 * d0.xyz;
	p1 += _Displacement * n1 * d1.xyz;
	p2 += _Displacement * n2 * d2.xyz;
	p3 += _Displacement * n3 * d3.xyz;
